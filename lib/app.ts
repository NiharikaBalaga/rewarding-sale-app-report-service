...
async function bootstrap() {
  ...
  const requiredEnvVariables = [
    'MONGODB_URI_REPORT',
    'MONGO_REPORT_DATABASE',
    'JWT_ACCESS_SECRET',
    'aws_sqs_access_key_id',
    'aws_sqs_secret_access_key',
    'aws_sns_access_key_id',
    'aws_sns_secret_access_key',
    'REPORT_TOPIC_SNS_ARN',
    'aws_region',
    'aws_sqs_queue_name',
    'aws_sqs_queue_url',
    'GOOGLE_MAPS_API_KEY',
  ...
  apiRouter.use('/report', routes); // Prefixing routes with '/post'
  app.use('/api', apiRouter); // Prefixing all routes with '/api'


  try {
    await mongoose.connect(process.env.MONGODB_URI_REPORT || '', {
      dbName: process.env.MONGO_REPORT_DATABASE,
    });
